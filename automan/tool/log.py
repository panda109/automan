...
class Log(object):
    ...
    def create_testcase_xml(self,xml_log):
        ...
        if self.finall_status() == False:
            ...
            result = 'pass'
        runtime = time.mktime(time.strptime(self.result[-1][1],"%a %b %d %H:%M:%S %Y")) - time.mktime(time.strptime(self.result[0][1],"%a %b %d %H:%M:%S %Y"))
        doc = Document()
    ...
    def parse_log(self,result,command):
        ...
        self.result.append((result,time.ctime()))
        if result == 0:
            print "result = PASS"
        else:
            print "result = FAIL"
